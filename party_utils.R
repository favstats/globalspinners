...
#   if(sets$cntry %in% country_codes & nrow(thedat)!=0){
  ...
  #   
cat_mapping <- readRDS(here::here("data/cat_mapping.rds"))

    color_dat <- cat_mapping %>% 
      count(party)  %>%
      mutate(color = NA) %>% 
      setColors() %>%
      rename(colors = color) %>%
...
# saveRDS(topspenders, "data/topspenders.rds")

maaaap <- readRDS(here::here("data/topspenders.rds")) %>% 
  select(page_id, page_category) %>% 
  as_tibble() %>% 
  left_join(cat_mapping) %>% 
  drop_na(party)
...
# print("oo")
all_spends <- readRDS(here::here("data/all_spends_clean.rds"))

election_dat30 <- election_dat30 %>% 
  filter(party != "Dismissed")

dollardat <- all_spends %>% 
  group_by(page_id) %>% 
  summarize(total_spend_formatted = sum(dollar_spend)) %>% 
  ungroup() %>% 
  rename(internal_id = page_id)

election_dat30 <- election_dat30 %>% 
  select(-contains("dollar_spend"), -contains("total_spend_formatted")) %>% 
  left_join(dollardat) %>% 
  distinct()
